...
export const Modal = ({ open, onClose, title, children, width }: ModalProps) => {
    ...
    return (
        <div
            onClick={onClose}
            className="fixed inset-0 bg-brown/45 backdrop-blur-md flex items-end justify-center z-[1000] animate-[fadeIn_.2s_ease]"
        >
            <div
            ...
            >
                ...
                <h2 className="font-fraunces text-xl font-bold text-brown m-0 mb-4">{title}</h2>
                {children}
            </div>
        </div>
    );
